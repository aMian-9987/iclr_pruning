...
def pruning_model_local(model, px):
    ...
    for name,m in model.named_modules():
        if isinstance(m, nn.Conv2d) or isinstance(module, nn.Linear):
...
if __name__ == '__main__':
    ...
    if run_mode in ['finetune','lottery','prune']:
        ...
        for i in range(len(models)):            
            ...
            pruning_model_local(model_tem)
            torch.save(model.state_dict(), f'./{experiment_dir}/origin_mask/model{i}.model')
    else:
        raise "wrong mode"
