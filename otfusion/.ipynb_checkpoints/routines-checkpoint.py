...
def get_trained_model(args, id, random_seed, train_loader, test_loader,re_models=None):
    torch.backends.cudnn.enabled = False
    torch.manual_seed(random_seed)
    if(re_models is None):
        network = get_model_from_name(args, idx=id)
    else:
        network = re_models[id]
    
    print("========================================")
    print("========================================")
    print("TEST:args.learning_rate,args.momentum",args.learning_rate,args.momentum)
    print("========================================")
    print("========================================")
    print("========================================")
...
def train(args, network, optimizer, train_loader, log_dict, epoch, model_id=-1):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        #if(mask is not None):
        #    prune_model_custom(network,mask)
...
def train_models(args, train_loader, test_loader,seed=None,re_models=None):
    networks = []
    accuracies = []
    for i in range(args.num_models):
        if seed is None:
            network, acc = get_trained_model(args, i, i, train_loader, test_loader,re_models)
        else:
            network, acc = get_trained_model(args, i, seed[i], train_loader, test_loader,re_models)
        networks.append(network)
        accuracies.append(acc)
        if args.dump_final_models:
...
def retrain_models(args, old_networks, train_loader, test_loader, config, tensorboard_obj=None, initial_acc=None, nicks=None):
    ...
    for i in range(len(old_networks)):
        ...
        if args.dataset.lower()[0:7] == 'cifar10':
            ...
            print("QWERTY: Enter Cifar 2, acc",acc)
        elif args.dataset.lower() == 'mnist':
