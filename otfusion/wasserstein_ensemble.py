...
def get_network_from_param_list(args, param_list, test_loader,return_loss=False):
    ...
    log_dict['test_losses'] = []
    if(return_loss):
        acc,loss = routines.test(args, new_network, test_loader, log_dict,return_loss=return_loss)
        return acc, new_network,loss
    else:
        acc = routines.test(args, new_network, test_loader, log_dict,return_loss=return_loss)
        return acc, new_network

def geometric_ensembling_modularized(args, networks, train_loader, test_loader, activations=None,return_loss=False):
    ...
    return get_network_from_param_list(args, avg_aligned_layers, test_loader,return_loss)
